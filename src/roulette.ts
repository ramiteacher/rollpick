...
export class Roulette extends EventTarget {
  ...
  public start() {
    if (this._isRunning || this._marbles.length === 0) return;
    this._isRunning = true;
    this.dispatchEvent(new CustomEvent('start', { detail: { count: this._marbles.length } }));
  ...
  /** 캔버스에 그려지는 당첨자 목록 팝업을 닫는다 (DOM 결과 창이 대신 보여줄 때) */
  public closeResultPopup() {
    this._renderer.closeResultPopup();
  }
  ...
  /** 확정된 당첨자 목록 (진행 중이거나 결과가 없으면 null) */
  public getResult(): { name: string; hue: number; rank: number }[] | null {
    if (!this._result) return null;
    return this._result.map((m, i) => ({ name: m.name, hue: m.hue, rank: this._winnerRange.start + i + 1 }));
  }

  /** 전체 순위 (골인 순서 + 아직 달리는 구슬) */
  public getRanking(): { name: string; hue: number; rank: number; finished: boolean }[] {
    const finished = this._winners.map((m, i) => ({ name: m.name, hue: m.hue, rank: i + 1, finished: true }));
    const running = this._marbles.map((m, i) => ({
      name: m.name,
      hue: m.hue,
      rank: this._winners.length + i + 1,
      finished: false,
    }));
